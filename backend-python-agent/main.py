...
Corrija o JSON atual para que seja 100% conforme o template. Mantenha TODO o conteúdo clínico gerado, apenas ajuste a estrutura, campos ausentes e tipos de dados. Retorne APENAS o JSON corrigido, sem explicações."""
            ...
            try:
                ...
                if revalidation_result["is_valid"]:
                    ...
                    logger.error("[CRITICAL] Correção automática falhou - JSON ainda inválido!")
                    logger.error(f"Problemas restantes: {revalidation_result}")
                    
                    # PARAR o processamento se a correção falhar
                    return False, {
                        "tema": tema,
                        "especialidade": especialidade,
                        "abordagem_usada": abordagem_id,
                        "validation_errors": revalidation_result
                    }, f"JSON gerado não atende aos padrões INEP mesmo após correção automática. Problemas: {revalidation_result['missing_required_fields']}, {revalidation_result['structural_issues']}"
...
async def generate_and_save_final_station(request: GenerateFinalStationRequest):
    ...
    if not validation_result["is_valid"]:
        ...
        correction_prompt = f"""# CORREÇÃO CRÍTICA DE JSON INEP - PROBLEMAS GRAVES DETECTADOS

Você deve corrigir COMPLETAMENTE este JSON para que seja 100% válido conforme o template INEP.
...
**INSTRUÇÕES CRÍTICAS:**
1. CORRIJA TODOS os campos ausentes listados acima
2. GARANTA que padraoEsperadoProcedimento.sinteseEstacao.itensAvaliacao existe e está completo
3. ADICIONE pontuacoes.parcialmenteAdequado em TODOS os itens de avaliação
4. REDUZA profundidade de impressos para máximo 2 níveis
5. MANTENHA todo o conteúdo clínico, apenas ajuste a estrutura
6. RETORNE o JSON COMPLETO E VÁLIDO

JSON CORRIGIDO (APENAS JSON, nada mais):"""
        ...
        try:
            ...
            if revalidation_result["is_valid"]:
                ...
                raise HTTPException(
                    status_code=500, 
                    detail=f"JSON gerado não atende aos padrões INEP mesmo após correção automática. "
                           f"Problemas: {revalidation_result['missing_required_fields']}, "
                           f"{revalidation_result['structural_issues']}"
                )
