...
Recomendações
- Use um subdomínio para o backend (ex.: `api.revalidaflow.com.br`) em vez de `www` para evitar conflitos com o frontend.
...
gcloud run deploy revalida-backend `
  ...
  --set-env-vars "NODE_ENV=production,FRONTEND_URL=https://www.revalidaflow.com.br" `
...
- Para mapear um domínio customizado (recomendado: `api.revalidaflow.com.br`), use `gcloud run domain-mappings create --service=revalida-backend --domain=api.revalidaflow.com.br --platform=managed --region=southamerica-east1`.
