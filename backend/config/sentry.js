...
const logger = require('../services/logger').child('sentry')

let pkg = { name: 'revalidaflow-backend', version: '0.0.0' }
try {
  pkg = require(path.join(__dirname, '..', 'package.json'))
} catch (error) {
  logger.warn('Não foi possível ler package.json para release Sentry:', error.message)
}
...
try {
  ...
  logger.warn('Profiling não disponível, continuando sem profiling')
}

function initSentry() {
  // Só inicializa se a variável de ambiente estiver definida
  if (process.env.SENTRY_DSN) {
    const defaultSampleRate = process.env.NODE_ENV === 'production' ? 0.1 : 1
    const configuredSampleRate = parseFloat(process.env.SENTRY_TRACES_SAMPLE_RATE || defaultSampleRate)
    const tracesSampleRate = Number.isFinite(configuredSampleRate) ? configuredSampleRate : defaultSampleRate

    const release =
      process.env.SENTRY_RELEASE ||
      `${pkg.name || 'revalidaflow-backend'}@${pkg.version || '0.0.0'}`
    ...
    logger.info('Sentry inicializado', { release, tracesSampleRate })
  } else {
    logger.debug('Sentry DSN não configurado; inicialização ignorada')
  }
}
