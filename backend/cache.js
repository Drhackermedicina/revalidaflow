...
async function getCachedData(key, fetchFunction, ttl = null) {
  try {
    ...
    if (cached !== undefined) {
      cacheStats.hits++;
      // Log removido para reduzir custos no Cloud Run
      return cached;
    }
    ...
    // Apenas log de erros críticos em produção
...
async function checkStationEditStatus(stationId, firestore) {
  ...
  return getCachedData(key, async () => {
    const stationRef = firestore.collection('estacoes_clinicas').doc(stationId);
...
async function checkMultipleStationsEditStatus(stationIds, firestore) {
  ...
  try {
    const stationRefs = uncachedIds.map(id => firestore.collection('estacoes_clinicas').doc(id));
