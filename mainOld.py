...
try:
    ...
    print("[DEBUG] Arquivos no diretório:", os.listdir(os.getcwd()))
    impressos_path = os.path.join(os.getcwd(), "impressos_validator.py")
    print(f"[DEBUG] impressos_validator.py existe? {os.path.exists(impressos_path)}")
    ...
    IMPRESSOS_VALIDATOR_AVAILABLE = False
    print(f"[ERROR] Erro inesperado ao importar impressos_validator: {e}")
...
# --- Variáveis Globais ---
db = None
firebase_mock_mode = True  # FORÇADO PARA TRUE - NUNCA SALVAR NO FIRESTORE
...
def initialize_local_memory_system():
    ...
    try:
        ...
        if gabarito_path and os.path.exists(gabarito_path):
            ...
            print(f"[SUCCESS] Gabarito carregado: {len(LOCAL_MEMORY_SYSTEM['gabarito_template'])} caracteres")
        else:
            # Tentar carregar diretamente do arquivo padrão
            default_gabarito = "gabaritoestacoes.json"
            print(f"[WARNING] Gabarito não encontrado em config, tentando: {default_gabarito}")
            if os.path.exists(default_gabarito):
                LOCAL_MEMORY_SYSTEM['gabarito_template'] = load_local_file(default_gabarito)
                print(f"[SUCCESS] Gabarito carregado do arquivo padrão: {len(LOCAL_MEMORY_SYSTEM['gabarito_template'])} caracteres")
            else:
                print(f"[ERROR] Gabarito não encontrado: {default_gabarito}")
                LOCAL_MEMORY_SYSTEM['gabarito_template'] = '{}'
...
def get_context_for_phase(phase_number):
    ...
    try:
        ...
        if not isinstance(sempre_carregar, list):
            sempre_carregar = []
        contexto_completo = ""
        ...
        if not isinstance(carregar_por_fase, dict):
            carregar_por_fase = {}
        ...
        if fase_str in carregar_por_fase:
            ...
            # Garantir que seja uma lista
            if not isinstance(arquivos_fase, list):
                arquivos_fase = []
        ...
        # Adicionar aprendizados do usuário
        aprendizados = LOCAL_MEMORY_SYSTEM.get('aprendizados', [])
        if aprendizados:
            learnings_context = format_learnings_for_context(aprendizados)
            if isinstance(learnings_context, str):
                contexto_completo += "\n\n" + learnings_context
            else:
                print(f"[WARNING] format_learnings_for_context retornou tipo inesperado: {type(learnings_context)}")
...
def debug_gabarito_loading():
    """Diagnóstico específico do carregamento do gabarito"""
    print("\n" + "="*60)
    print("🔍 DIAGNÓSTICO DO CARREGAMENTO DO GABARITO")
    print("="*60)
    
    # 1. Verificar se arquivo físico existe
    gabarito_path = "gabaritoestacoes.json"
    if os.path.exists(gabarito_path):
        with open(gabarito_path, 'r', encoding='utf-8') as f:
            content = f.read()
        print(f"✅ Arquivo gabaritoestacoes.json existe ({len(content)} caracteres)")
    else:
        print(f"❌ Arquivo gabaritoestacoes.json NÃO encontrado em: {os.path.abspath(gabarito_path)}")
    
    # 2. Verificar sistema de memória
    if LOCAL_MEMORY_SYSTEM:
        gabarito_loaded = LOCAL_MEMORY_SYSTEM.get('gabarito_template', '')
        print(f"Sistema híbrido ativo: {len(gabarito_loaded)} caracteres carregados")
        if gabarito_loaded == '{}':
            print("⚠️ PROBLEMA: Gabarito vazio no sistema híbrido!")
        else:
            print("✅ Gabarito carregado com sucesso no sistema híbrido!")
    else:
        print("❌ Sistema híbrido não inicializado")
    
    # 3. Verificar config
    config_path = "memoria/config_memoria.json"
    if os.path.exists(config_path):
        with open(config_path, 'r', encoding='utf-8') as f:
            config = json.load(f)
        gabarito_config = config.get('sistema_memoria', {}).get('estrutura', {}).get('gabarito_template', '')
        print(f"Config aponta para: {gabarito_config}")
    else:
...
def _try_firebase_connection_simple(credential_path, description):
    ...
    try:
        ...
        # firebase_mock_mode = False  # COMENTADO - MANTÉM MODO LOCAL
        return True
...
def load_rules_from_firestore():
    ...
    if hybrid_success:
        ...
        # Executar diagnóstico do gabarito
        debug_gabarito_loading()
...
def validate_json_against_template(generated_json: dict, template_path: str = "gabaritoestacoes.json") -> dict:
    ...
    try:
        ...
        if "padraoEsperadoProcedimento" in generated_json:
            ...
            required_padrao_fields = ["idChecklistAssociado", "sinteseEstacao", "itensAvaliacao"]
            ...
            # Verificar itensAvaliacao (no mesmo nível que sinteseEstacao)
...
def _sanitize_impressos(data: Any) -> List[Dict[str, Any]]:
   ...
   for item in data:
       ...
       tipo_impresso = sanitized_item.get("tipoConteudo")  # Correção: usar "tipoConteudo" em vez de "tipo"
...
def _calculate_max_depth(obj: Any) -> int:
    """
    ...
    if isinstance(obj, dict):
        if not obj:
            return 1
        return 1 + max((_calculate_max_depth(v) for v in obj.values()), default=0)
    elif isinstance(obj, list):
        if not obj:
            return 1
        return 1 + max((_calculate_max_depth(i) for i in obj), default=0)
    else:
        return 0
                            ...
                            for i, item in enumerate(value[:5]):  # Examinar primeiros 5 para diagnóstico
                                if isinstance(item, dict):
                                    ...
                                    if "conteudo" in item:
                                        ...
                                        if conteudo_depth > 2:
                                            error_msg = f"impressos[{i}].conteudo VIOLATION: profundidade {conteudo_depth} > 2"
                                            validation_result["structural_issues"].append(error_msg)
                                            validation_result["is_valid"] = False
                                            print(f"[ERROR] {error_msg}")
                                            print(f"[ERROR] Conteúdo problemático: {str(conteudo)[:500]}...") # Log preview
                                            ...
                                            if isinstance(conteudo, dict):
                                                for sub_key, sub_value in conteudo.items():
                                                    ...
                                                    if sub_depth > 1: # Profundidade 1 é permitida (chave: valor_simples)
                                                        print(f"[ERROR] Causa provável: impressos[{i}].conteudo.{sub_key} com profundidade {sub_depth}")
                                                        print(f"[ERROR] Valor problemático: {str(sub_value)[:500]}...")
...
**INSTRUÇÕES CRÍTICAS PARA IMPRESSOS MÉDICOS:**
Para TODOS os impressos com tipoConteudo "lista_chave_valor_secoes", o campo "conteudo" deve conter OBRIGATORIAMENTE uma estrutura "secoes" com arrays de objetos, NUNCA strings JSON serializadas.
...
"conteudo": {{
  "secoes": [
    {{
      ...
      "itens": [
        {{"chave": "Pressão Arterial", "valor": "120/80 mmHg"}},
        {{"chave": "Frequência Cardíaca", "valor": "72 bpm"}}
      ]
    }}
  ]
}}

ESTRUTURA INCORRETA (NÃO FAÇA ISTO):
"conteudo": {{
  "SINAIS VITAIS": "string JSON serializada"
}}

**SUA TAREFA:**
Gere o código JSON completo para a estação sobre {request.tema} em {request.especialidade}, seguindo rigorosamente a proposta, as regras, os padrões INEP encontrados, as regras aprendidas do usuário e o molde fornecidos. GARANTA que TODOS os impressos médicos sigam a estrutura "secoes" -> "itens" sem serialização JSON."""
...
async def generate_single_station_internal(tema: str, especialidade: str, abordagem_id: str, enable_web_search: bool = False):
    """
    ...
    try:
        ...
        is_valid_impressos = True
        validation_errors_impressos = []
        if IMPRESSOS_VALIDATOR_AVAILABLE:
            logger.info("🏥 Iniciando validação avançada de impressos médicos...")
            
            try:
                is_valid_impressos, validation_errors_impressos, estacao_corrigida = validar_impressos_estacao(json_output)
                
                if not is_valid_impressos:
                    logger.warning(f"⚠️ Impressos com problemas detectados: {len(validation_errors_impressos)} erros")
                    
                    # Log dos erros para análise
                    for error in validation_errors_impressos:
                    ...
                    if MONITORING_SYSTEM.get('active'):
                        MONITORING_SYSTEM['metrics']['validation_warnings'] += len(validation_errors_impressos)
        ...
        logger.info("🧹 Limpando JSON final antes de salvar...")
        if isinstance(json_output, dict):
            # Remover campos de prompt que não devem ser salvos
            json_output.pop("promptIA", None)
            json_output.pop("gabarito", None)
            logger.info("Campos 'promptIA' e 'gabarito' removidos do JSON final.")

        # **SALVAMENTO APENAS LOCAL** - Removido Firestore
        logger.info("💾 Salvando estação APENAS localmente...")
        new_station_id = str(uuid.uuid4())
        local_stations_dir = "estacoes_geradas"
        os.makedirs(local_stations_dir, exist_ok=True)
        
        # Criar nome de arquivo mais descritivo usando o tema
        tema_clean = tema.replace(' ', '_').replace('/', '_').replace('\\', '_').lower()
        tema_clean = ''.join(c for c in tema_clean if c.isalnum() or c in '_-')
        ...
        station_with_metadata = {
            **json_output,
            ...
            "created_by": "sistema_local",
            "source": "local_only",
            "tema_original": tema,
            "especialidade_original": especialidade
        }
        
        # Salvar JSON local com nome descritivo
        local_file = os.path.join(local_stations_dir, f"{tema_clean}_{new_station_id[:8]}.json")
        ...
        return True, {
            ...
            "validation_status": "valid" if is_valid_impressos else "warning",
            "validation_warnings": validation_errors_impressos,
...
async def generate_and_save_final_station(request: GenerateFinalStationRequest):
    """
    Orquestra a Fase 3, gerando o JSON final e salvando APENAS localmente
    ...
    logger.info("Iniciando geração de estação com Gemini Pro (salvamento apenas local)",
    ...
    # **VALIDAÇÃO AVANÇADA DE IMPRESSOS MÉDICOS**
    if IMPRESSOS_VALIDATOR_AVAILABLE:
        logger.info("🏥 Iniciando validação e correção de impressos médicos...")
        try:
            is_valid, validation_errors, estacao_corrigida = validar_impressos_estacao(json_output)
            if not is_valid:
                logger.warning(f"⚠️ Validação encontrou {len(validation_errors)} problemas. A estação foi corrigida automaticamente.")
                for error in validation_errors:
                    logger.warning(f"   - Erro de validação: {error}")
            else:
                logger.info("✅ Estação passou na validação de impressos.")
            json_output = estacao_corrigida  # Usar a estação corrigida
        except Exception as validation_error:
            logger.error(f"Erro crítico na validação de impressos: {validation_error}")
            # Decide-se continuar com o JSON original se a validação falhar
    else:
        logger.warning("⚠️ Sistema de validação de impressos não disponível - pulando validação.")
    ...
    try:
        ...
        # Criar nome de arquivo mais descritivo usando o tema
        tema_clean = request.tema.replace(' ', '_').replace('/', '_').replace('\\', '_').lower()
        tema_clean = ''.join(c for c in tema_clean if c.isalnum() or c in '_-')
...
async def analyze_station_endpoint(request: AnalyzeStationRequest):
    # SEMPRE usar busca local - firebase_mock_mode forçado como True
    if firebase_mock_mode or not AGENT_RULES or not db:
        try:
            local_stations_dir = "estacoes_geradas"
